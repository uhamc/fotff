...
func (m *Manager) build(pkg string) error {
	...
	cmd := fmt.Sprintf("source ~/.bashrc; mkdir -p %s && cd %s && repo init -u https://gitee.com/openharmony/manifest.git", m.BuildServerConfig.BuildWorkSpace, m.BuildServerConfig.BuildWorkSpace)
	...
	cmd = fmt.Sprintf("source ~/.bashrc; cd %s && repo sync -c --no-tags --force-remove-dirty", m.BuildServerConfig.BuildWorkSpace)
	if out, err := utils.RunCmdViaSSH(m.BuildServerConfig.Addr, m.BuildServerConfig.User, m.BuildServerConfig.Passwd, cmd); err != nil {
		logrus.Error(out)
		return fmt.Errorf("remote: repo sync error: %s", err)
	}
	cmd = fmt.Sprintf("source ~/.bashrc; cd %s && %s", m.BuildServerConfig.BuildWorkSpace, preCompileCMD)
	if out, err := utils.RunCmdViaSSH(m.BuildServerConfig.Addr, m.BuildServerConfig.User, m.BuildServerConfig.Passwd, cmd); err != nil {
		logrus.Error(out)
		return fmt.Errorf("remote: pre-compile command error: %s", err)
	}
	cmd = fmt.Sprintf("source ~/.bashrc; cd %s && %s", m.BuildServerConfig.BuildWorkSpace, compileCMD)
	if out, err := utils.RunCmdViaSSH(m.BuildServerConfig.Addr, m.BuildServerConfig.User, m.BuildServerConfig.Passwd, cmd); err != nil {
		logrus.Error(out)
